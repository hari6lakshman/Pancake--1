...
export function AffirmationGenerator() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setAffirmation('');
    try {
      const result = await generateAffirmation(values);
      setAffirmation(result.affirmation);
    } catch (error) {
      console.error('Error generating affirmation:', error);
      setAffirmation('Today, I will be kind to myself and trust my journey.');
    } finally {
      setIsLoading(false);
    }
  }
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="e.g., confidence, self-compassion, finding peace" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <div className="flex items-center justify-center p-6 bg-primary/10 border-2 border-dashed border-primary/20 rounded-xl min-h-[300px] lg:min-h-0">
        {isLoading ? (
          ...
          <blockquote className="text-center">
            ...
            <p className="text-2xl font-headline italic text-foreground/90">
              "{affirmation}"
            </p>
          </blockquote>
        ) : (
          <div className="text-center text-muted-foreground">
            ...
            <p>Your personalized affirmation will appear here.</p>
          </div>
        )}
      </div>
    </div>
